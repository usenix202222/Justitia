...
const extractIR = function(IR, ast, args) {
    ...
    contractDef.name = IR.ContractCategory;
    ...
    for (var x in IR.Payments) {
        ...
        if (pay_IR.From.length == 0 || IR.BuyerName.length == 0) {
            ...
            curBuyerIndex = BuyerName2Index[pay_IR.From];
        } else {
        ...
        if (pay_IR.TimeLimit != undefined) {
            stmts.splice(cur_index, 0, createRequire(genBinaryOperationFromIR(pay_IR.TimeLimit)));
            cur_index += 1;
        }
...
function createRequire(binaryOp) {
    var obj = 
    {
        "type": "ExpressionStatement",
        "expression": {
